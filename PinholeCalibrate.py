...
class PinholeCalibrator():
    def __init__(self,img_dic,image_size):
        ...
        self._left_D = np.zeros((5,1))      # Left camera distortion vector     : 4*1
        self._right_D = np.zeros((5,1))     # Right camera distortion vector    : 4*1
    ...
    def stereo_calibrate(self,detail=True):
        ...
        # ret, self._right_K, self._right_D,_,_  = cv2.calibrate(
        ...
        # ret, self._left_K, self._left_D,_,_  = cv2.calibrate(
        #     objectPoints=self.object_points,
        ...
        # flags +=  cv2.fisheye.CALIB_FIX_SKEW
        # flags += cv2.fisheye.CALIB_FIX_INTRINSIC
        ret, self._left_K, self._left_D, self._right_K, self._right_D, self._R, self._T, E, F   = cv2.stereoCalibrate(
            # objectPoints=self.object_points,
            ...
            # flags = flags,
            # # criteria=criteria
            self.object_points, self.left_corners, self.right_corners, self._left_K, self._left_D,
                                                                 self._right_K, self._right_D, self._image_size,flags=0

        )

        print(f'cv2.stereoCalibrate: {ret}')
        print(self._left_K)
        print(self._left_D)
        print(self._right_K)
        print(self._right_D)
    ...
    def stereo_rectify(self, detail=False):
        ...
        flags = cv2.fisheye.CALIB_RECOMPUTE_EXTRINSIC
        self._left_R, self._right_R, self._left_P, self._right_P, self._Q, _, _ = cv2.stereoRectify(
            self._left_K,
            self._left_D,
            self._right_K,
            self._right_D,
    ...
    def create_rectify_map(self):
        ...
        self._left_rectify_map_x, self._left_rectify_map_y = cv2.initUndistortRectifyMap(
            self._left_K,
            self._left_D,
            self._left_R,
            self._left_P,
        ...
        self._right_rectify_map_x, self._right_rectify_map_y = cv2.initUndistortRectifyMap(
            self._right_K,
            self._right_D,
            self._right_R,
            self._right_P,
